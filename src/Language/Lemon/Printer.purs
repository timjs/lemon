...
doDecl name (Value typ expr) doc = vcat
...
doExpr = case _ of
  Atom a ->
    doAtom a
  Lam ( pat ** _ ) inner ->
    parens $ hsep [ text "\\", doPattern pat, text "->", doExpr inner ]
  App func arg ->
    hsep [ parens $ doExpr func, parens $ doExpr arg ]
  Let decls inner ->
    vcat
      [ text "let"
...
doStmt = case _ of
  Set pat expr ->
    hsep [ text "let", doPattern pat, text "=", doExpr expr, text "in" ]
  Bind pat expr ->
    bind pat expr
  Do expr ->
    bind PIgnore expr
  Par brns ->
    vcat $ Array.map par brns
  When brns ->
    vcat
      [ text ">>>"
  ...
  where
    bind pat expr =
      hsep [ doExpr expr, text ">>=", text "\\", doPattern pat, text "->" ]
    pair2 ( pred ** stmts ) =
      tuple [ doExpr pred, doStmt stmt ]
    pair3 ( name ** pred ** stmts ) =
      tuple [ text name, doExpr pred, doStmt stmt ]
    par stmts =
      vcat
        [ text "also"
...
cons conv head tail =
  brackets $ hsep [ conv head, text ":", conv tail ]

nil = text "[]"

just conv inner =
  hsep [ text "Just", parens $ conv inner ]
...
doPattern = case _ of
  ...
  PVar name ->
    text name
  PJust inner ->
    just doPattern inner
  PNothing -> nothing
  PCons head tail ->
    cons doPattern head tail
  PNil -> nil
  PRecord fields ->
    tuple $ map (text << fst) fields
  PIgnore -> text "_"

doAlternative :: Alternative Expr -> Doc -> Doc
doAlternative ( pat ** expr ) doc = vcat
...
doType = case _ of
  ...
  TVar name ->
    text name
  TMaybe inner ->
    hsep [ text "Maybe", doType inner ]
  TList inner ->
    hsep [ text "Array", doType inner ]
  TRecord fields ->
    tuple $ map (doType << snd) fields
  TTask inner ->
    hsep [ text "Task", doType inner ]
  TArrow left right ->
    hsep [ parens $ doType left, text "->", doType right ]
